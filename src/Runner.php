...
class Runner
{
    ...
    public function __construct(TsqmConfig $config)
    {
        ...
        $this->scheduler = $config->getRunScheduler();
     ...
     * @throws Exception 
     */
    public function performRun(Run $run): RunResult
    {
        $task = $run->getTask();

        if ($task->getForceAsync() || $task->getScheduledFor() > new DateTime()) {
            $this->scheduler->schedule($run, $task->getScheduledFor());
            $this->logger->debug("Run scheduled for " . $task->getScheduledFor()->format('Y-m-d H:i:s.v'), ['run' => $run]);
        ...
        try {
            $this->performTask($run, $task, $history);
            $this->finishRun($run);
        } catch (StopTheRun $e) {
     ...
     * @return string[]
     */
    public function getScheduledRunsIds(DateTime $until, int $limit): array
    {
     ...
     * @throws Exception
     */
    private function performTask(Run $run, Task $task, Generator $history, bool $inGenerator = false)
    {
                ...
                while ($cb++ < self::GENERATOR_CB_LIMIT) {
                    if ($generator->valid()) {
                        ...
                        if ($childTask instanceof Task) {
                            $childTaskResult = $this->performTask($run, $childTask, $history, true);
        ...
        // Here we handle all the exceptions from the task code
            ...
            if ($retryAt) {
                ...
                $this->scheduler->schedule($run, $retryAt);
